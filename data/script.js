...
async function loadAreaIndex() {
  ...
  if (!res.ok) throw new Error(`Не удалось загрузить data/index.json (HTTP ${res.status})`);
  AREA_INDEX = await res.json();
  EXAM_AREAS = AREA_INDEX.map(a => a.examArea);
}

/**
 * Показывает понятную ошибку вместо тихого «зависания» на пустом экране,
 * если манифест data/index.json не загрузился (нет сети, файл не задеплоен,
 * опечатка в пути и т.п.) — раньше необработанное исключение в init() просто
 * останавливало выполнение до отрисовки кнопок областей, и человек видел
 * заголовок без единой подсказки, что пошло не так.
 */
function showStartupError(err) {
  ...
  msg.className = 'hint centered startup-error';
  const detail = err && err.message ? err.message : String(err);
  msg.textContent = `Не удалось загрузить список вопросов (${detail}). Проверьте подключение к интернету и обновите страницу (Ctrl+Shift+R / Cmd+Shift+R).`;
  box.appendChild(msg);
}
